...
public class RequestCreator {
	 ...
	 * request until the {@link ImageView} has been measured.
	 * <p/>
	...
	public Bitmap get() throws IOException {
		...
		return forRequest(picasso.context, picasso, picasso.dispatcher,
				picasso.cache, picasso.stats, action,
	...
	public void fetch() {
		...
		if (data.hasImage()) {
			...
			Action action = new FetchAction(picasso, finalData,
					skipMemoryCache, key);
	...
	public void into(Target target) {
		...
		Drawable drawable = placeholderResId != 0 ? picasso.context
				.getResources().getDrawable(placeholderResId)
				: placeholderDrawable;
		...
		Action action = new TargetAction(picasso, target, finalData,
	...
	public void into(ImageView target, Callback callback) {
		...
		if (!data.hasImage()) {
			picasso.cancelRequest(target);
			PicassoDrawable.setPlaceholder(target, placeholderResId,
					placeholderDrawable);
			return;
		}
		...
		if (!skipMemoryCache) {
			...
			if (bitmap != null) {
				...
				PicassoDrawable.setBitmap(target, picasso.context, bitmap,
						MEMORY, noFade, picasso.debugging);
				if (callback != null) {
					callback.onSuccess();
				}
				return;
		...
		Action action = new ImageViewAction(picasso, target, finalData,
				skipMemoryCache, noFade, errorResId, errorDrawable, requestKey,
				callback);
