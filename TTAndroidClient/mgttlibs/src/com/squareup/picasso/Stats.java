...
class Stats {
	...
	private static final String STATS_THREAD_NAME = Utils.THREAD_PREFIX
			+ "Stats";
	...
	void performBitmapDecoded(long size) {
		...
		averageOriginalBitmapSize = getAverage(originalBitmapCount,
	...
	void performBitmapTransformed(long size) {
		...
		averageTransformedBitmapSize = getAverage(originalBitmapCount,
				totalTransformedBitmapSize);
	}

	StatsSnapshot createSnapshot() {
		return new StatsSnapshot(cache.maxSize(), cache.size(), cacheHits,
				cacheMisses, totalDownloadSize, totalOriginalBitmapSize,
				totalTransformedBitmapSize, averageDownloadSize,
	...
	private void processBitmap(Bitmap bitmap, int what) {
		// Never send bitmaps to the handler as they could be recycled before we
		// process them.
