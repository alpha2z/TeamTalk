...
class MediaStoreBitmapHunter extends ContentStreamBitmapHunter {
	...
	MediaStoreBitmapHunter(Context context, Picasso picasso,
	...
	Bitmap decode(Request data) throws IOException {
		...
		if (data.hasSize()) {
			...
			calculateInSampleSize(data.targetWidth, data.targetHeight,
					picassoKind.width, picassoKind.height, options);

			Bitmap result = getThumbnail(contentResolver, id,
	...
	enum PicassoKind {
		MICRO(MICRO_KIND, 96, 96), MINI(MINI_KIND, 512, 384), FULL(
				FULL_SCREEN_KIND, -1, -1);
